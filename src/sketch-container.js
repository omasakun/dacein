...
export class SketchContainer extends React.Component {
  ...
  setupInspector = (props) => {
    ...
    this.onMouseDownInspector = (e) => {
      ...
      const [mx, my] = [Math.floor(e.clientX - bbox.left), Math.floor(e.clientY - bbox.top)]
    ...
    this.onMouseMoveInspector = (e) => {
      ...
      if (this.props.optimiser) {
        ...
        this.props.setHighlight(meta ? { start: meta.lineStart - 2, end: meta.lineEnd - 1 } : null)
      }
    }
  ...
  tick = () => {
    ...
    if (isPlaying) {
      this.currentState = immer(this.currentState, (draft) => sketch.update(draft, this.events))
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      xor(Object.keys(nextProps.sketch.initialState), Object.keys(this.currentState)).length > 0
    ) {
    ...
    if (nextProps.historyIdx !== this.props.historyIdx && !nextProps.isPlaying) {
