    ...
    <div className='relative h-100 overflow-scroll'>
      <CodeMirror
        ...
        onCursor={(e) => {
          ...
          const coords = e.charCoords({ line: cursor.line, ch: token.start }, 'local')
          ...
          setPicker({
            ...
            type: token.type,
            value: token.type === 'number' ? Number(token.string) : token.string,
            coords,
            text: {
              line: cursor.line,
              start: token.start,
            ...
            onChange={(value) => {
              ...
              const newCode = editorCode
                ...
                .map((codeLine, i) => {
                  ...
                  return (
                    codeLine.substr(0, start) + value + codeLine.substr(start + tokenLength.current)
                  )
                })
                .join('\n')
