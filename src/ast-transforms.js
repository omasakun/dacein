...
export const processRequire = (code) => {
  ...
  const codeWithoutRequire = code
    .split('\n')
    .map((line, i) => {
      const shouldBeBlank = requires.some(({ start, end }) => i >= start && i <= end)
  ...
  const finalCode = `
    ${requires.map(({ name, arg }) => `window.require("${arg}").then(${name} => {`).join('\n')}
  ...
  types.visit(ast, {
    ...
    visitLiteral: function (path) {
      if (isNumber(path.value.value)) {
        ...
        if (get(path, ['parentPath', 'value', 'object', 'name']) === '__constants') {
          return false
        }
